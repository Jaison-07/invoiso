    ...
    try {
      ...
      final results = await Future.wait([
        ...
        isEditing
            ...
            : ref.read(invoiceRepositoryProvider).peekNextInvoiceNumber(
              invoiceType, date: _selectedOrderDate), // 2
          ...
          TextField(
            ...
            onTap: () async {
              ...
              if (picked != null) {
                ...
                if (!isEditing) {
                  final invoiceNumber = await InvoicePdfServices
                      .peekNextInvoiceNumber(invoiceType,
                          date: _selectedOrderDate);
                  if (mounted) {
                    setState(() => currentInvoiceNumber = invoiceNumber);
                  }
                }
                await _loadPreviousBalanceDue(selectedCustomer);
              }
      ...
      children: [
        ...
        if (_isTaxEnabled) ...[
          ...
          SegmentedButton<bool>(
            ...
            selected: {_showCgstSgst},
            onSelectionChanged: (selection) async {
              if (!mounted) return;
              final value = selection.first;
              setState(() => _showCgstSgst = value);
              await ref.read(settingsRepositoryProvider).setSetting(
                    SettingKey.showCgstSgst,
                    value.toString(),
                  );
            },
          ),

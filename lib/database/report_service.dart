...
class ReportService {
  ...
  static Future<RevenueKpi> getRevenueSummary(DateTime from, DateTime to,
    ...
    final collected = await _getCollectedBetween(
      from,
      to,
      currencyCode: currencyCode,
    );
    if (rows.isEmpty && collected == 0) return RevenueKpi.empty;

    double billed = 0;
    for (final r in rows) {
      billed += r.total;
    }
    final outstanding = (billed - collected).clamp(0.0, double.infinity);
    ...
    return RevenueKpi(
      ...
      collected: collected,
      outstanding: outstanding,
      avgInvoiceValue: rows.isEmpty ? 0.0 : billed / rows.length,
      profit: profit,
    );
  }

  static Future<double> _getCollectedBetween(
    DateTime from,
    DateTime to, {
    String? currencyCode,
  }) async {
    ...
    final rows = await db.rawQuery(
      "SELECT COALESCE(SUM(ip.amount_paid), 0.0) AS collected "
    ...
    return (rows.first['collected'] as num?)?.toDouble() ?? 0.0;
  }
  ...
  static Future<List<CustomerStatement>> getCustomerStatements(
    ...
    for (final entry in byCurrency.entries) {
      ...
      var running = opening.clamp(0.0, double.infinity).toDouble();
      final lines = drafts.map((draft) {
        running = (running + draft.billed - draft.received)
        ...
        final outstanding = (opening + invoiced - paid)
          .clamp(0.0, double.infinity)
          .toDouble();
        ...
        openingBalance: opening.clamp(0.0, double.infinity).toDouble(),
        invoiced: invoiced,
        paid: paid,
  ...
  static String exportCustomerStatementsCsv(
    ...
    for (final statement in statements) {
      blocks.add(buildQuotedCsv([
        ...
        ['Received', statement.paid.toStringAsFixed(2)],
        ['Outstanding', statement.closingBalance.toStringAsFixed(2)],
        ...
        for (final line in statement.lines)
          [
            ...
            line.reference,
            line.description,
            line.billed.toStringAsFixed(2),
            line.received.toStringAsFixed(2),

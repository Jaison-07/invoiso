...
class InvoiceService {
  ...
  static Future<String> generateNextInvoiceNumber(String type,
    ...
    final financialYearStart = invoiceDate.month >= 4
        ? invoiceDate.year
        : invoiceDate.year - 1;
    final financialYearEnd = financialYearStart + 1;
    final yearLabel =
        '${(financialYearStart % 100).toString().padLeft(2, '0')}-'
        '${(financialYearEnd % 100).toString().padLeft(2, '0')}';

    if (type.toLowerCase() == 'invoice') {
      final rows = await db.query(
        'invoices',
        columns: ['invoice_number'],
        where: "type = ? AND date >= ? AND date < ? AND invoice_number IS NOT NULL",
        whereArgs: [
          type,
          '${financialYearStart.toString().padLeft(4, '0')}-04-01',
          '${financialYearEnd.toString().padLeft(4, '0')}-04-01',
        ],
      );
      var nextNumber = 1;
      for (final row in rows) {
        final value = row['invoice_number'] as String?;
        final match = RegExp(r'^(\d+)/').firstMatch(value ?? '');
        final number = int.tryParse(match?.group(1) ?? '');
        if (number != null && number >= nextNumber) nextNumber = number + 1;
      }
      return '$nextNumber/$yearLabel';
    }
